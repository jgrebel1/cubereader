...
class Form(QMainWindow):
    ...
    def fit_cube(self):
        if self.spectrum_holder.cube_fitting:
            self.stop_fit = True
        self.threadPool.append( GenericThread(self.fit_cube_process_2))
        (rows,columns,slices) = np.shape(self.data.ycube[...])
        #self.progress_bar = self.fit_cube_progress_bar(rows)

        self.threadPool[len(self.threadPool)-1].start()

    def fit_cube_process_2(self):
        #locker = QtCore.QMutexLocker(self.spectrum_holder.cube_mutex)
        ...
        for result in self.lview.map(sleep_and_return, range(10)):
            print result

        
        
    def fit_cube_process_3(self, i):
        column_count = 0
        for j in np.arange(columns):
                ...
                for peak in spectrum.peaks.peak_list:
                    peak_list.append(peak.get_spec())
                cube_peaks.append(peak_list)
                norm_int_res = self.get_normalized_integrated_residuals(spectrum)
                cube_residuals.append(norm_int_res)
                if self.stop_fit is True:
                    return
                value += 1
                #self.update_progress(value)
        return cube_peaks,cube_residuals
    ...
    def fit_cube_progress_bar(self, rows):
...
def par():
    #parallel computing
    clients = parallel.Client(profile = 'hyperspy')
    #self.lview = clients.load_balanced_view()
    dview = clients.direct_view()
    dview.block = False
    print dview.apply(sum, [1, 2, 3])
